...
public class JacobiSolve2 {
    ...
    public void comeig(int n,
        ...
        while ((itcount <= itlimit) && (!mark)) {
            ...
            tem = tem + Math.abs(A[1][1]) + Math.abs(Z[1][1]);
            tau = tau + tem;
            tep = Math.abs(A[1][1]) + Math.abs(Z[1][1]);
            diag = diag + tep;
            Rvec[1] = tem + tep;

            tem = tem + Math.abs(A[2][2]) + Math.abs(Z[2][2]);
            tau = tau + tem;
            tep = Math.abs(A[2][2]) + Math.abs(Z[2][2]);
            diag = diag + tep;
            Rvec[2] = tem + tep;

            tem = tem + Math.abs(A[3][3]) + Math.abs(Z[3][3]);
            tau = tau + tem;
            tep = Math.abs(A[3][3]) + Math.abs(Z[3][3]);
            diag = diag + tep;
            Rvec[3] = tem + tep;

            tem = tem + Math.abs(A[4][4]) + Math.abs(Z[4][4]);
            tau = tau + tem;
            tep = Math.abs(A[4][4]) + Math.abs(Z[4][4]);
            diag = diag + tep;
            Rvec[4] = tem + tep;
            ...
            for (k = 1; k <= n1; k++) {
                ...
                if (i != k) {
                    ...
                    //  для вычисления правых собственных векторов
                    for (j = 1; j <= n; j++) {
                        tep = A[j][k];
  //                      System.out.println("tep = " + tep + " A[" + j + "][" + k + "];");
                        A[j][k] = A[j][i];
                        A[j][i] = tep;
                        tep = Z[j][k];
                        Z[j][k] = Z[j][i];
